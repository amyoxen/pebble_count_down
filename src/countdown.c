...
bool decrement_timer () {
  ...
  if (seconds > 0 ) {
    ...
    if (seconds == 300) {
      ...
      text_layer_set_text_color(text_min_layer, GColorYellow);
      }
    //Vibration when 2 mins left.
    if (seconds == 120) {
      vibes_enqueue_custom_pattern(min52_vibe);
      text_layer_set_text_color(text_min_layer, GColorRed);
      }
  } else {
    seconds = 0;
  }
...
static void handle_second_tick(struct tm *t, TimeUnits units_changed) {
  ...
  if (timer_running) {
    if (decrement_timer()) {
      ...
      text_layer_set_text_color(text_min_layer, GColorGreen);
...
void handle_init(void) {
  ...
  text_layer_set_text_color(text_label_layer, GColorGreen);
